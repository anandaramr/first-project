...
function Card({heading,comp,source}){
    ...
    return(
        <div className="font-raleway flex justify-evenly">
            <img className="size-[40%] rounded-3xl m-6 hover:scale-[1.05] duration-200" onClick={click} src={source} alt="heading"/>
            <div className=" flex justify-start flex-col items-start">
            <h2 className="font-semibold text-2xl mx-10 my-14">{heading}</h2>
            <p className="line-clamp-6 mx-10">{comp}</p>
            <button onClick={click} className="group mx-10 my-5 p-2 font-poppins border-2 rounded border-slate-900 hover:bg-slate-900 hover:text-white duration-150">
                {heading} 
